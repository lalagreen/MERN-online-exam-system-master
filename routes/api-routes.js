...
var Admin = require('../db/schema/adminschema');
var Scores = require('../db/schema/testscore');
const Timer = require('../db/schema/timerschema');
...
router.post('/updatetimer', (req, res) => {
    ...
    Timer.find({}, (err, docs) => {
        if (err) {
            ...
            Timer.findByIdAndUpdate(docs[0]._id, {
                timer: obj.timer 
            }, (err) => {
                if (err) {
                    console.log(err);
                ...
                if (flag == 1) {
                    ...
                    if (flag == 2) {
                        ...
                        for (j; j < docs.length; j++) {
                            Scores.findByIdAndUpdate(docs[j]._id, {
                                rank: docs[j].rank + 1
                            }, (err) => {
                                if (err) {
                                    console.log(err);
